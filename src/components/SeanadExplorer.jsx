...
import { partiesPalette } from "../seanad/data/partiesPalette.js";
import ChamberMap from "../seanad/components/ChamberMap.jsx";
import chamberSvg from "../seanad/data/chamber.svg?raw";
import membersJson from "../seanad/data/members.json";
...
export default function SeanadExplorer() {
  useIframeResize();
  ...
  useEffect(() => {
    async function init() {
      const seatingRowsRaw = await loadCsv(
        `${import.meta.env.BASE_URL}seanad/seatAssignmentsHistory.csv`,
      );
  ...
  const visibleSeats = useMemo(() => {
    return seats.filter((seat) => {
      ...
      const matchesParty =
        partyFilter === null || getSeatParty(seat) === partyFilter;
